...
func (m *notifyConsumer) Consume(message kafka2.Message) error {
	push := logic.PushNotification{}
	err := json.Unmarshal(message.Value, &push)
	if err != nil {
		return err
	}
	m.m.RLock()
	if conn, ok := m.comets[push.Server]; ok {
		m.m.RUnlock()
		client := comet.NewCometClient(conn)
		_, err := client.PushNotification(context.Background(), &comet.PushNotificationRequest{
...
func (m *notifyConsumer) watchComet() {
